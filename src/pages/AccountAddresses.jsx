    ...
    <div className="rounded-[2rem] bg-white p-5 shadow-[0_18px_70px_-50px_rgba(15,23,42,0.55)] sm:p-8">
      ...
      {showForm && (
        <form onSubmit={onSave} className="mt-6 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
          ...
          <button disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-medium text-white hover:bg-emerald-700 disabled:opacity-60 sm:col-span-2">
      ...
      {loading ? (
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-slate-200 p-8 text-center text-slate-500">No saved addresses.</div>
      ) : (
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {addresses.map((a, i) => (
            <div key={i} className="rounded-[1.5rem] border border-slate-100 bg-slate-50 p-4 transition hover:bg-white hover:shadow-sm">
